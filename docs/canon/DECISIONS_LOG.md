...
- **Correction (2026-08-10, during implementation):** §2.3 `gate_status` values were corrected from the drafting placeholder `{approved, review, blocked}` (conflated with `content_items.status`) to the real `citemind_scores.gate_status` enum `{pending, analyzing, passed, warning, blocked}` (migration 82 CHECK). gate_factor now keys on `passed`=1.0 / `warning`=0.5 / `blocked`=0.0, with not-yet-scored states (`pending`, `analyzing`) = 0.0. No coefficient changed (0.35/0.45/0.70 and all other formulas unaffected).
...
## D039 — SAGE Daily Brief: recovered as an org-level daily grounded narrative

- **Date:** 2026-08-10
- **Decision ID:** D039
- **Area:** Foundational model (SAGE output) / Command Center
- **Decision:** Ratify the SAGE Daily Brief (`SAGE_DAILY_BRIEF.md`) as an **org-level, daily, cross-pillar** narrative on the Command Center + mobile Today tab, summarizing real SAGE signals (`sage_proposals`/`sage_signals`, action-stream, `evi_snapshots` delta, CiteMind citation movement) into a 2–4 sentence orientation + top action. Generation is LLM-primary with a deterministic stub fallback (grounded-only, both modes), honest empty state when signals are sparse, nightly + on-demand cadence. Persona/role-targeted variants, Exec Digest/Media Briefing rebuild, and multi-brand briefs are OUT of scope.
- **Rationale:** The feature was designed with real product weight (the "open the app and know your highest-leverage move" moment — the OS heartbeat) but drifted: `sageDailyBriefService.ts` was roadmapped and never built, the demo text was removed leaving the Command Center card permanently empty, and onboarding still promises a brief the pipeline cannot deliver. The intelligence inputs it needs now exist (SAGE signals/proposals, EVI, action-stream, CiteMind — all built in the Wave-2 recovery), so a grounded generator is now honest (summarization of real rows), not fabrication — the same LLM+stub pattern already shipping in `sageProposalGenerator`. A dependency audit + live prod verification confirmed the scheduler foundation works (prod Redis is plaintext `redis://` by design and connects; do NOT flip to `rediss://`), narrowing the real prerequisites to: a nightly all-orgs SAGE signal-scan, brief persistence, and a shared-LLM-budget degradation rule.
- **Scope note:** Persona/role briefs require a canonized user-persona model that does not exist (the "12+ personas" figure was never canonized; no `USER_PERSONAS.md`) — deferred until personas are canonized. The drifted `/app/pr/intelligence` "weekly" surface is reconciled/retired in favor of the daily Command Center brief.
- **Authority basis:** Founder-ratified 2026-08-10.
- **Cleanup follow-through (2026-08-12):** Removed the orphaned, never-consumed `apps/dashboard/src/components/pr-intelligence/` (`PRAIRecommendations`, `PRContinuityLinks` — Sprint-S95/DS-v2, built on a stale pillar ontology and functionally superseded by the shipping SAGE PR action-queue and this Daily Brief) plus the dangling, never-referenced `PR_INTELLIGENCE_WIRED` flag. The `/app/pr/intelligence` route is retained as a redirect to the Command Center for old bookmarks. Affirms scope: **PR-specific intelligence is delivered via the SAGE PR action-queue (Copilot) + this Daily Brief / Command Center Intelligence Canvas + integrated CiteMind — not a standalone PR Intelligence surface.** Any future PR-intelligence depth is to be built into those canonical homes on the current pillar model + DS v3.1, reconciled with the SAGE queue — not by reviving the retired surface.
- **Cross-refs:** D033 (Model Integrity Charter); `SAGE_DAILY_BRIEF.md`; `SAGE_v2.md`; `SAGE_OPERATING_MODEL.md`; `sageProposalGenerator` (honesty pattern).

## D040 — "Growth" plan renamed to "Scale" and repositioned above Pro

- **Date:** 2026-08-20
- **Decision ID:** D040
- **Area:** Plans / Billing / Canon hygiene
- **Decision:** The self-serve billing tier formerly named **Growth** is renamed **Scale** and is the **top self-serve tier, priced ABOVE Pro**. The ratified ladder is **Starter ($199) < Pro ($599) < Scale ($1,199) < Enterprise (sales-led)**. This supersedes the retired pre-launch model in which "Growth" was the **mid** tier priced **below** Pro. The plan slug is `scale`; the legacy slug `growth` is retained as a **backward-compatible alias** (no hard cutover) so nothing breaks mid-rollout.
- **Rationale:** The old canon (CRAFT_EXECUTION_MODEL, CITEMIND_SYSTEM) and the code caps encoded Growth **below** Pro — an inversion the founder flagged. PR #178 already fixed the code caps so the top self-serve tier exceeds Pro on every guardrail. This decision aligns the **name** (Growth→Scale) and **position** (above Pro) with that ratified ceiling, and with the live pricing page (`pravado.io`) which already sells Scale at $1,199/mo ($959/mo annual). Renaming the plan has **no effect on SAGE** — SAGE's "Growth" is a distinct concept (the **G** in Signal·Authority·**Growth**·Exposure) and is explicitly out of scope here.
- **Backward-compatibility design (deliberate, keep until Render env fully cut over):**
  - `planLimitsService.normalizePlanSlug()` maps `'growth' → 'scale'`; `getPlanLimits()` / `getPlanCeiling()` inherit it. Legacy `growth` callers resolve Scale limits unchanged.
  - `governanceGateways.normalizeTier()` treats `'growth'` as a legacy alias (→ `'pro'` tier bucket) alongside `'scale'`.
  - `priceIdMap`: canonical env is **`STRIPE_PRICE_SCALE`**; `scale` falls back to **`STRIPE_PRICE_GROWTH`** when the Scale env is unset (rollout), and the `growth` slug normalizes to `scale`. Reverse webhook lookup returns `scale` for the legacy price.
- **Code changes (this PR):**
  - API: `planLimitsService.ts` (`scale` key + `normalizePlanSlug`), `priceIdMap.ts` (`STRIPE_PRICE_SCALE` + fallback + normalize), `governanceGateways.ts` (`case 'scale'` + legacy `growth`), `bootstrapStripeBilling.ts` (slug/name/description/log = Scale), `scripts/configureRenderProduction.ts` (push `STRIPE_PRICE_SCALE`, keep `STRIPE_PRICE_GROWTH` fallback).
  - Validators: `packages/validators/src/env.ts` adds `STRIPE_PRICE_SCALE` (legacy `STRIPE_PRICE_GROWTH` kept, commented as rollout fallback).
  - Dashboard: marketing pricing page, marketing home (plan card + comparison line), settings/billing page (`id`/`name`), legal/terms tier list, PR `DistributionDecisionMatrix` badge, and the PR guardrails route (`DAILY_PITCH_CAPS`: `growth:50` → `scale:350` repositioned above Pro=200; hardcoded placeholder `plan`; also fixed a stale `@see AUTOMATE_v2.md` → `CRAFT_v2.md`).
  - Tests: `priceIdMap.test.ts` and `stripeWebhookProcessing.test.ts` re-pinned to the canonical `scale`/`STRIPE_PRICE_SCALE` **plus new explicit legacy-rollout regression cases** (a `STRIPE_PRICE_GROWTH`-only env still resolves Scale). Behavioral fixtures across `applyCheckoutCompletion`, `craftExecutionService`, `craftGuardrailService`, `modeService`, `planLimitsEnforcement`, `planLimitsReconciliation`, `billingCheckout`, and the `billing-page` spec renamed `growth→scale` / `plan-growth→plan-scale`, kept internally self-consistent (all green).
  - Canon: `SAGE_ARCHITECTURE.md` token line renamed Growth→Scale and stale values aligned to the ratified code figures (Starter 2.5M / Pro 5M / Scale 50M).
- **Canon Files Impacted / HELD reconciliation (flagged, NOT silently rewritten per founder's "hold judgment-call rewrites"):** `CRAFT_EXECUTION_MODEL.md` (§2.2, §6.2, §6.3, §12.5) and `CITEMIND_SYSTEM.md` (§2.6, §3.9, §4, §6.3) still render plan-tier tables in the **retired** `Starter → Growth → Pro` order (Growth below Pro). Both docs now carry a **top-of-file ⚠️ reconciliation banner** marking those tables non-authoritative for tiering and pointing to `planLimitsService.ts` + this entry. Re-ordering the columns to `Starter | Pro | Scale | Enterprise` and setting the **per-tier CiteMind cap values for Scale** (not yet ratified — e.g. audio briefings/mo, citation scans/day) is a HELD follow-up requiring founder review. Also flagged: `PLANS_LIMITS_ENTITLEMENTS.md` (canon priority 7) still lists only **Starter / Pro / Enterprise** and never defined a 4th self-serve tier — the Scale tier spec should be added there when the held reconciliation is done.
- **Prod-ops runbook (gated to founder; NOT executed by this PR):**
  1. **DB:** `UPDATE billing_plans SET slug='scale', name='Scale' WHERE slug='growth';` — verified **0 orgs** currently on the plan, so zero customer impact. (`org_billing_state.plan_id` is a UUID FK, not the slug string, so the row's id is unchanged.)
  2. **Stripe:** rename the live product "Pravado Growth" → "Pravado Scale" (price IDs unchanged). `bootstrapStripeBilling.ts` is idempotent and now emits `STRIPE_PRICE_SCALE=…`.
  3. **Render (prod + staging):** set `STRIPE_PRICE_SCALE` to the existing Growth price ID value; leave `STRIPE_PRICE_GROWTH` in place until the next deploy confirms Scale resolves, then remove it and drop the fallback in `priceIdMap.ts` + `env.ts`.
- **Authority basis:** Founder-ratified 2026-08-20 ("changing the name of the plan should have no negative effect on SAGE… All plans include SAGE, CRAFT, etc." + "yes, proceed. Make sure you thoroughly document in case we need it later.").
- **Cross-refs:** PR #178 (Scale cap-inversion fix); D037 (Enterprise post-launch); `PLANS_LIMITS_ENTITLEMENTS.md`; `planLimitsService.ts`; `priceIdMap.ts`; Canon Drift Register.

## D041 — First-session routing gates on onboarding COMPLETION, not org existence

- **Date:** 2026-08-21
- **Decision ID:** D041
- **Area:** Onboarding / Acquisition funnel / Auth routing
- **Decision:** The post-auth redirect gate routes a signed-in user to `/onboarding/ai-intro` until their org has **completed onboarding** (`orgs.completed_onboarding_at` set), rather than as soon as an org merely **exists**. The Silo Tax audit magic link now routes through `/callback` (the canonical convergence that runs the gate) instead of landing directly on `/app/command-center`.
- **Rationale:** The audit funnel (the primary acquisition path) pre-creates the org + `org_members` row inside `/scan`, so `session-check` saw `hasOrg=true` and sent audit users straight to the Command Center — **bypassing the onboarding wizard and its SAGE activation**. Result: audit users (and anyone who abandoned onboarding mid-wizard) landed in an **unseeded** Command Center showing EVI 0/Critical, while their real audit EVI + pillar gaps sat unused in `audit_sessions`, and the activation that seeds real EVI + SAGE proposals never ran. Gating on completion routes these users through the existing, tested 7-step wizard → real in-product EVI + proposals. Chosen over "seed the dashboard directly at claim time" because it reuses the tested activation pipeline, additionally captures GSC/journalists (high-value setup the audit never collected), produces the honest in-product EVI (not the marketing-audit heuristic), and requires no new seeding code.
- **Implementation:** (1) `apps/dashboard/.../api/auth/session-check/route.ts` — when `hasOrg`, consult `GET /api/v1/onboarding/status` and route on `completed`; **fail-safe = treat as completed on any backend error** so an established user is never trapped in the wizard (preserves prior behavior on a hiccup). (2) `apps/api/src/routes/siloTaxAudit/index.ts` — magic-link `redirectTo` → `https://app.pravado.io/callback`. (3) Onboarding prefill: `routes/onboarding/index.ts` `/status` returns `audit_prefill` (latest `audit_sessions` brand_url + competitor_urls for the org); the wizard prefills brand domain + competitors from it (only when the org has none yet), so audit users don't re-enter what they gave.
- **Verification:** typecheck clean (api + dashboard). Behavior needs a live magic-link round-trip before relying on it (no unit test harness for the callback chain).
- **Authority basis:** Founder-directed 2026-08-21 ("resolve any remaining issues found"); approach (route-through-wizard) recommended by Claude and left to Claude's judgment.
- **Cross-refs:** TRACK-0A-COLD-START-UNBLOCK (session-check origin); `SESSION_PRIMER.md` (2026-08-21 launch-readiness gaps).

## D042 — Share of Model exists in code (closes scope item B5)

- **Date:** 2026-08-21
- **Decision ID:** D042
- **Area:** SEO/AEO / CiteMind (Engine 3) / Foundational metric
- **Decision:** Implement **Share of Model** — the Layer-3 "moat" metric defined in `SEO_AEO_PILLAR_CANON.md` §4 — as a real, computed product metric. This closes the code half of the open scope item **B5** (DECISIONS_LOG entry noting "Share-of-Model not existing in code"). No new metric definition is invented: the computation is the canon formula verbatim, **Share of Model = Brand AI Citations / (Brand + Competitor AI Citations) × 100**, overall + per-topic, with a period-over-period trend delta.
- **Data source (real, no fabrication):** `citation_monitor_results` (migration 83), written by the CiteMind citation monitor (Engine 3 — `citationMonitor.ts`, with a BullMQ worker `citationMonitorWorker.ts` + an on-demand `/monitor/scan` trigger). A row is a **brand** citation when `brand_mentioned = true`, a **competitor** citation when `!brand_mentioned && mention_type = 'competitor'`; rows that are neither count only toward the sampled-query size. **Honest-empty contract:** when the monitor has produced no rows, `available = false`; when rows exist but nothing was cited, `shareOfModel = null` (NOT a fake 0% — zero would falsely imply "measured, and you have none"). Share populates as the monitor runs.
- **Implementation:** `apps/api/src/services/citeMind/shareOfModelService.ts` (pure `computeShareOfModel(current, previous, days)` + DB reader `getShareOfModel`, current-vs-previous period split for the trend); `GET /api/v1/citemind/share-of-model` (flag-gated on `ENABLE_CITEMIND`, honest-empty when off); dashboard proxy `/api/seo/share-of-model` + `useShareOfModel` hook; rendered as the top "Share of Model" hero + per-topic bars in `SeoOverviewPanel` (re-adds the block that PR #183 removed as fabricated, now real). 6-case service unit test pins the math + honest-empty + trend.
- **Still open (follow-ups, NOT in scope here):** (1) canon also specifies Share of Model in the **Command Center Intelligence Canvas** as the primary cross-pillar metric — reuse `useShareOfModel` there. (2) Per-competitor breakdown (canon viz shows Competitor A/B); the monitor currently records aggregate `mention_type='competitor'`, not per-competitor identity — a richer breakdown needs the monitor to attribute the competing entity. (3) Confirm the citation-monitor worker is scheduled in prod so real data flows (metric is honest-empty until it runs). (4) Citation sentiment / placement sub-metrics (canon §4C) not built.
- **Authority basis:** Founder-directed 2026-08-21 ("Let's proceed to Share of Model next"). Canon: `SEO_AEO_PILLAR_CANON.md` §4.
- **Cross-refs:** scope item B5 (D0-series band-nomenclature entry); `SEO_AEO_PILLAR_CANON.md` §4; migration 83 (`citation_monitor_results`); PR #183 (removed the fabricated hero this replaces with real data).
