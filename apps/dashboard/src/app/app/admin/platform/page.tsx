      ...
      {data?.redis_unavailable && (
        <div className="bg-brand-cyan/10 border border-brand-cyan/20 rounded-lg px-4 py-3 mb-6">
          <p className="text-sm text-brand-cyan">
            BullMQ queues are disabled — Redis unavailable
          </p>
        </div>
      )}
              ...
              {data.queues.map((q) => (
                <tr key={q.name} style={{ background: '#0A0A0F' }}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <span
                      className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded ${statusBadge(q)}`}
                    >
                      {q.failed > 10 || q.waiting > 100
                        ? 'Critical'
                        : q.failed > 0
                          ? 'Warning'
                          : 'Healthy'}
