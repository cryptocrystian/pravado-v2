...
interface Overview {
  ...
  sage: {
    proposals_generated_7d: number;
    proposals_accepted_7d: number;
...
function StatusPill({ label, status }: { label: string; status: string }) {
  ...
  return (
    <span
      className={`px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider rounded border ${color}`}
    >
      {label}: {status}
    </span>
  );
...
function MetricCard({
  ...
  return (
    <div
      ...
      style={{ background: '#13131A', borderColor: '#1F1F28' }}
    >
...
export default function AdminOverviewPage() {
  ...
  const anyDegraded =
    data.system.database !== 'ok' || data.system.redis !== 'ok';
    ...
    <div className="p-8 max-w-6xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <MetricCard
          ...
          subtitle={
            data.orgs.total > 0
              ? `${Math.round((data.orgs.onboarding_complete / data.orgs.total) * 100)}% of total`
              : '--'
          }
        />
        <MetricCard
          ...
          value={
            data.sage.acceptance_rate > 0
              ? `${Math.round(data.sage.acceptance_rate * 100)}%`
              : '--'
          }
