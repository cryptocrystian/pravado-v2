...
export default function AdminBetaPage() {
  ...
  const [pendingInviteRequestId, setPendingInviteRequestId] = useState<
    string | null
  >(null);
  ...
  async function handleSendInvite() {
    await fetch('/api/admin/beta/invite', {
      ...
      body: JSON.stringify({
        email: inviteEmail,
        requestId: pendingInviteRequestId,
      }),
    });
      ...
      {tab === 'waitlist' && (
        <>
          ...
          <div className="flex gap-1 mb-4">
            ...
            <span className="ml-2 text-[11px] text-white/30 self-center">
              {total} total
            </span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: '#13131A' }}>
                  <th className="text-left px-4 py-3 text-[11px] text-white/40 uppercase tracking-wider">
                    Email
                  </th>
                  <th className="text-left px-4 py-3 text-[11px] text-white/40 uppercase tracking-wider">
                    Company
                  ...
                  <th className="text-right px-4 py-3 text-[11px] text-white/40 uppercase tracking-wider">
                    Actions
                  </th>
                </tr>
              </thead>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center gap-1 justify-end">
                        ...
                        {(r.status === 'pending' ||
                          r.status === 'approved') && (
                          <button
                            onClick={() => openInviteModal(r.email, r.id)}
              ...
              <tbody className="divide-y" style={{ borderColor: '#1F1F28' }}>
                {codes.map((c) => (
                  <tr key={c.id} style={{ background: '#0A0A0F' }}>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-[10px] font-bold uppercase ${c.used ? 'text-semantic-success' : 'text-white/30'}`}
                      >
                        {c.used ? 'Yes' : 'No'}
                      </span>
                    </td>
