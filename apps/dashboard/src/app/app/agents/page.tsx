...
function AIInsightBanner({
  ...
  const borderColor =
    type === 'success' ? 'border-l-semantic-success' : 'border-l-brand-cyan';
  const bgColor =
    type === 'success' ? 'bg-semantic-success/5' : 'bg-brand-cyan/5';
...
async function getAgents(): Promise<AgentDefinition[]> {
  try {
    ...
    const data = await backendFetch<{ agents: AgentDefinition[] }>(
      '/api/v1/agents'
    );
    return data?.agents || [];
  } catch (error) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-3xl font-bold text-white-0 mb-2">
                AI Agents
              </h2>
              <p className="text-muted">
                Specialized AI agents available for playbook workflows and
                automation
              </p>
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {prAgents.map((agent) => (
                <div
                  key={agent.id}
                  className="panel-card p-5 hover:border-brand-cyan/30 transition-colors"
                >
                  <div className="flex items-start gap-2 mb-2">
                    <AIDot />
                    <h4 className="text-lg font-semibold text-white-0">
                      {agent.name}
                    </h4>
                  </div>
                  ...
                  <div className="space-y-2 text-xs">
                    <div>
                      <span className="font-medium text-slate-6">
                        Capabilities:
                      </span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {agent.capabilities.map((cap) => (
                          <span
                            key={cap}
                            className="px-2 py-0.5 bg-brand-cyan/10 text-brand-cyan rounded"
                          >
                            {cap}
                          </span>
                        ))}
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {contentAgents.map((agent) => (
                <div
                  key={agent.id}
                  className="panel-card p-5 hover:border-brand-iris/30 transition-colors"
                >
                  <div className="flex items-start gap-2 mb-2">
                    <AIDot />
                    <h4 className="text-lg font-semibold text-white-0">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {seoAgents.map((agent) => (
                <div
                  key={agent.id}
                  className="panel-card p-5 hover:border-semantic-success/30 transition-colors"
                >
                  <div className="flex items-start gap-2 mb-2">
                    <AIDot />
                    <h4 className="text-lg font-semibold text-white-0">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {generalAgents.map((agent) => (
                <div
                  key={agent.id}
                  className="panel-card p-5 hover:border-brand-magenta/30 transition-colors"
                >
                  <div className="flex items-start gap-2 mb-2">
                    <AIDot />
                    <h4 className="text-lg font-semibold text-white-0">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {agent.capabilities.map((cap) => (
                          <span
                            key={cap}
                            className="px-2 py-0.5 bg-slate-5 text-slate-6 rounded"
                          >
                            {cap}
                          </span>
                        ))}
