...
import {
  ...
  getPillarFromSource,
  getAffectedPillars,
} from '@/components/PillarContinuityLinks';
import {
  listSimulations,
  getStats as getSimulationStats,
  ...
  return (
    <div className="panel-card p-4 hover:shadow-lg transition-shadow duration-200 group">
      <div className="flex items-center gap-3">
        <div
          className={`w-10 h-10 rounded-xl ${accentColor}/10 flex items-center justify-center ${accentColor} group-hover:scale-105 transition-transform`}
        >
          {icon}
        </div>
    ...
    <div className="panel-card p-5 hover:shadow-lg transition-all duration-200 group">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div
            className={`w-8 h-8 rounded-lg ${accentColor}/10 flex items-center justify-center ${accentColor}`}
          >
            {icon}
          </div>
        ...
        <ul className="space-y-3">
          {items.slice(0, 3).map((item) => (
            <li key={item.id} className="flex items-start gap-3 group/item">
              <span
                className={`w-1.5 h-1.5 rounded-full ${accentColor} mt-2 shrink-0`}
              />
              <div className="flex-1 min-w-0">
              ...
              {item.badge && (
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${item.badgeColor || 'bg-slate-5 text-muted'}`}
                >
                  {item.badge}
                </span>
              )}
...
function AIInsightBanner({
  ...
  const affectedPillars = getAffectedPillars(
    sourcePillar,
    isRisk || false,
    isOpportunity || false
  );
  ...
  return (
    <div
      className={`panel-card p-4 border-l-4 ${
        isRisk
          ? 'border-l-semantic-danger bg-semantic-danger/5'
          : isOpportunity
            ? 'border-l-semantic-success bg-semantic-success/5'
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            ...
            {affectedPillars.length > 0 && (
              <>
                <span className="text-[10px] text-slate-6 uppercase ml-1">
                  Affects
                </span>
...
function createReasoningContext(
  ...
  return {
    ...
    triggerDescription:
      insight.description ||
      'AI-detected signal based on continuous monitoring',
    sourcePillar,
    relatedPillars,
...
// Activity Item
function ActivityItem({ insight }: { insight: ExecDashboardInsight }) {
      ...
      {insight.severityOrImpact > 0 && (
        <span
          className={`text-xs px-2 py-0.5 rounded-full shrink-0 ${
            insight.severityOrImpact >= 80
              ? 'badge-confidence-high'
              : insight.severityOrImpact >= 50
...
function AIRecommendationsPanel({
  ...
  const draftScenarios = scenarios.filter(
    (s) => s.status === 'configured' || s.status === 'paused'
  );
  ...
  if (recommendations.length === 0) {
    recommendations.push({
      ...
      description:
        'Your dashboard is healthy. Explore deeper insights in the Executive Command Center.',
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          ...
          <div className="text-center p-2 rounded-lg bg-semantic-success/5">
            <p className="text-lg font-bold text-semantic-success">
              {scenarioStats.byStatus?.completed || 0}
            </p>
...
export default function DashboardClient({ userName }: DashboardClientProps) {
  ...
  const [narrative, setNarrative] = useState<ExecDashboardNarrative | null>(
    null
  );
  const [topInsight, setTopInsight] = useState<ExecDashboardInsight | null>(
    null
  );
  ...
  const [scenarioStats, setScenarioStats] =
    useState<AIScenarioSimulationStats | null>(null);
  const [narrativesList, setNarrativesList] = useState<UnifiedNarrative[]>([]);
  const [narrativeStats, setNarrativeStatsData] =
    useState<NarrativeStats | null>(null);
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (dashboardsRes.success && dashboardsRes.data?.dashboards?.length) {
        ...
        const detailRes = await execDashboardApi.getDashboard(
          firstDashboard.id
        );
        ...
        if (detailRes.success && detailRes.data) {
          ...
          if (detailRes.data.topInsights?.length) {
            const riskInsight = detailRes.data.topInsights.find(
              (i) => i.isRisk
            );
            const opportunityInsight = detailRes.data.topInsights.find(
              (i) => i.isOpportunity
            );
            setTopInsight(
              riskInsight || opportunityInsight || detailRes.data.topInsights[0]
            );
          }
        }
      }
  ...
  const prInsights = insights.filter(
    (i) =>
      i.sourceSystem === 'media_monitoring' || i.sourceSystem === 'outreach'
  );
  const contentInsights = insights.filter(
    (i) => i.sourceSystem === 'press_releases' || i.sourceSystem === 'pitches'
  );
  const riskInsights = insights.filter((i) => i.isRisk);
  const opportunityInsights = insights.filter((i) => i.isOpportunity);
  ...
  const seoKpi =
    kpis.find((k) => k.category === 'seo') || getKpiValue('media_performance');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
          <KPICard
            ...
            trendValue={
              prKpi?.metricTrend?.changePercent
                ? `${prKpi.metricTrend.changePercent}%`
                : undefined
            }
            icon={Icons.pr}
          ...
          <KPICard
            ...
            trendValue={
              contentKpi?.metricTrend?.changePercent
                ? `${contentKpi.metricTrend.changePercent}%`
                : undefined
            }
            icon={Icons.content}
          ...
          <KPICard
            ...
            trendValue={
              seoKpi?.metricTrend?.changePercent
                ? `${seoKpi.metricTrend.changePercent}%`
                : undefined
            }
            icon={Icons.seo}
          ...
          <KPICard
            label="Scenarios"
            value={
              scenarioStats?.totalSimulations?.toLocaleString() ||
              scenarios.length.toString()
            }
            trend={
              scenarioStats?.byStatus?.running &&
              scenarioStats.byStatus.running > 0
                ? 'up'
                : 'flat'
            }
            trendValue={
              scenarioStats?.byStatus?.running
                ? `${scenarioStats.byStatus.running} active`
                : undefined
            }
            icon={Icons.scenario}
          ...
          <KPICard
            label="Narratives"
            value={
              narrativeStats?.totalNarratives?.toLocaleString() ||
              narrativesList.length.toString()
            }
            trend={
              narrativeStats?.byStatus?.draft &&
              narrativeStats.byStatus.draft > 0
                ? 'up'
                : 'flat'
            }
            trendValue={
              narrativeStats?.byStatus?.draft
                ? `${narrativeStats.byStatus.draft} drafts`
                : undefined
            }
        ...
        <div className="grid grid-cols-12 gap-6">
            ...
            <div className="panel-card p-6">
              ...
              {narrative ? (
                <div className="prose prose-invert prose-sm max-w-none">
                  ...
                  {narrative.risksSection && (
                    <div className="mt-4 p-3 rounded-lg bg-semantic-danger/5 border border-semantic-danger/20">
                      <h4 className="text-sm font-medium text-semantic-danger mb-2">
                        Key Risks
                      </h4>
                ...
                <p className="text-muted text-sm">
                  No AI narrative generated yet. Click Refresh to generate
                  insights.
                </p>
              )}
                ...
                <div className="mt-6 pt-4 border-t border-border-subtle">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {riskInsights.slice(0, 2).map((insight) => (
                      ...
                      >
                        {Icons.conflict}
                        {insight.title.slice(0, 40)}
                        {insight.title.length > 40 ? '...' : ''}
                      </span>
                    ))}
              ...
              <IntelligenceCard
                ...
                items={insights
                  .filter(
                    (i) =>
                      i.category === 'seo' ||
                      i.sourceSystem === 'media_performance'
                  )
                  .slice(0, 3)
                  .map((i) => ({
                    id: i.id,
          ...
          <div className="col-span-12 lg:col-span-4">
            ...
            {/* AI Recommendations Panel (S92 Enhancement) */}
            <AIRecommendationsPanel insights={insights} scenarios={scenarios} />
