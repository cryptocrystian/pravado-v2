...
const MOCK_INCIDENT: CrisisIncident = {
  ...
  summary:
    'This is a demonstration of the Crisis Hub interface. When a real incident is detected, this page will show relevant details and recommended actions.',
  recommendedActions: [
...
function SeverityBadge({ severity }: { severity: CrisisIncident['severity'] }) {
  ...
  return (
    <span
      className={`px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider rounded-full border ${styles[severity]}`}
    >
      {severity}
    </span>
  );
...
function StatusBadge({ status }: { status: CrisisIncident['status'] }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[13px] font-medium rounded-lg ${styles[status]}`}
    >
      {icons[status]}
...
function RiskBadge({ level }: { level: 'high' | 'medium' | 'low' }) {
  ...
  return (
    <span className={`text-[13px] font-medium ${styles[level]}`}>
      {level} risk
    </span>
  );
}
...
export default function CrisisHubPage() {
  ...
  const incident = useMemo(() => {
    ...
    return {
      ...MOCK_INCIDENT,
      id: incidentId,
      title: `Crisis Incident: ${incidentId}`,
    };
  }, [incidentId]);
      ...
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center max-w-md">
          <div className="w-16 h-16 mx-auto mb-6 rounded-2xl bg-brand-magenta/10 flex items-center justify-center">
            <svg
              className="w-8 h-8 text-brand-magenta"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <p className="text-[15px] text-white/55 mb-6">
            No active incident selected. Crisis incidents will appear here when
            detected by SAGE or created manually.
          </p>
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[15px] text-white/55 ml-9">
            Incident ID:{' '}
            <span className="text-white/70 font-mono">{incident.id}</span>
          ...
          <div className="p-5 rounded-xl bg-[#0D0D12] border border-[#1A1A24]">
            ...
            <div className="flex items-center justify-between mt-3">
              <span className="text-[13px] text-white/40">
                {holdingStatement.length} characters
              </span>
              <div className="flex items-center gap-2">
                <button
                  type="button"
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <div className="flex-1">
                  <div className="text-sm font-medium text-white group-hover:text-brand-magenta transition-colors">
                    PR Inbox
                  </div>
                  <div className="text-[13px] text-white/55">
                ...
                <svg
                  className="w-4 h-4 text-white/30 group-hover:text-white/55 transition-colors"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              >
                ...
                <div className="flex-1">
                  <div className="text-sm font-medium text-white group-hover:text-brand-iris transition-colors">
                    Pitch Pipeline
                  </div>
                  <div className="text-[13px] text-white/55">
                    Pause or adjust active pitches
                  </div>
                </div>
                <svg
                  className="w-4 h-4 text-white/30 group-hover:text-white/55 transition-colors"
              ...
              >
                ...
                <div className="flex-1">
                  <div className="text-sm font-medium text-white group-hover:text-brand-cyan transition-colors">
                    Coverage Monitor
                  </div>
                  <div className="text-[13px] text-white/55">
                    Track emerging coverage
                  </div>
                </div>
                <svg
                  className="w-4 h-4 text-white/30 group-hover:text-white/55 transition-colors"
