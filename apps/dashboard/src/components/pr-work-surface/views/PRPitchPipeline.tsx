...
function PipelineCard({ item, onAction }: PipelineCardProps) {
  ...
  const ctaStyles = {
    primary:
      'bg-brand-magenta text-white/95 hover:bg-brand-magenta/90 shadow-[0_0_12px_rgba(232,121,249,0.25)]',
    secondary:
      'bg-white/5 text-white/70 hover:text-white hover:bg-white/10 border border-white/10',
    success:
      'bg-semantic-success/20 text-semantic-success hover:bg-semantic-success/30 border border-semantic-success/30',
  };

  return (
    <div
      className={`p-3 rounded-xl bg-panel border ${stageConfig.borderColor} hover:border-slate-5 transition-all duration-200 ${item.isOverdue ? 'ring-1 ring-semantic-danger/30' : ''}`}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-2">
        <span
          className={`px-2 py-0.5 text-[13px] font-medium rounded ${stageConfig.bgColor} ${stageConfig.color}`}
        >
          {item.contact.outlet}
        </span>
        <span className="text-[13px] text-white/50">
          ...
          {item.isOverdue && (
            <span className="text-semantic-danger font-medium">(Overdue)</span>
          )}
        </span>
      </div>
      ...
      {item.followUpWindow && (
        <div className="mb-3 flex items-center gap-2 text-[13px]">
          ...
          <span className="text-brand-magenta font-medium">
            {item.followUpCount}/{item.maxFollowUps}
          </span>
...
function StageColumn({ stage, items, onAction }: StageColumnProps) {
  ...
  return (
    <div className="flex-shrink-0 w-72">
      {/* Column Header */}
      <div
        className={`mb-3 p-3 rounded-xl border ${config.borderColor} ${config.bgColor} ${hasActiveItems && config.glowOnActive ? 'shadow-[0_0_16px_rgba(232,121,249,0.15)]' : ''}`}
      >
        <div className="flex items-center justify-between">
          ...
          <span
            className={`px-2 py-0.5 text-[13px] font-bold rounded-full bg-slate-1 ${config.color}`}
          >
            {items.length}
          </span>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-xl bg-panel border-l border-border-subtle flex flex-col">
        ...
        <div className="p-6 border-b border-border-subtle">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-xl ${prAccent.bg} flex items-center justify-center`}
              >
                <svg
                  className={`w-5 h-5 ${prAccent.text}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {/* Recipient */}
          <div>
            <label className="text-[11px] font-bold uppercase tracking-wider text-white/50 mb-2 block">
              To
            </label>
        ...
        <div className="p-6 border-t border-border-subtle">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4 text-xs text-white/50">
              <span>
                Personalization:{' '}
                <span className="text-brand-magenta font-medium">--</span>
              </span>
              <span>
                Topic Currency:{' '}
                <span className="text-semantic-success font-medium">85%</span>
              </span>
            </div>
          </div>
          <div className="flex items-center gap-3">
    ...
    <div className="space-y-6">
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-panel">
                ...
                <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-white/50">
                  Contact
                </th>
                <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-white/50">
                  Subject
            ...
            <tbody className="divide-y divide-border-subtle">
              {items.map((item) => {
                ...
                return (
                  <tr
                    key={item.id}
                    className="bg-slate-1 hover:bg-slate-2 transition-colors"
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-1 text-[13px] font-medium rounded border ${stageConfig.bgColor} ${stageConfig.color} ${stageConfig.borderColor}`}
                      >
                        {stageConfig.label}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-white/55">
                      {item.daysSinceLastActivity}d
                      {item.isOverdue && (
                        <span className="text-semantic-danger ml-1 font-medium">
                          !
                        </span>
                      )}
                    </td>
      ...
      <div className="p-4 rounded-xl bg-semantic-warning/5 border border-semantic-warning/20">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="text-sm font-semibold text-semantic-warning">
              Relationship-First Approach
            </h4>
            <p className="text-xs text-white/55 mt-1">
              All pitch sends require individual action to maintain relationship
              quality. Bulk send is not available by design.
