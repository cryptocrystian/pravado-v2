...
const STAGE_CONFIG = {
  ...
  warm: {
    color:
      'bg-semantic-warning/15 text-semantic-warning ring-semantic-warning/30',
    label: 'Warm',
    icon: (
      <svg
        className="w-3 h-3"
  ...
  engaged: {
    color:
      'bg-semantic-success/15 text-semantic-success ring-semantic-success/30',
    label: 'Engaged',
    icon: (
      <svg
        className="w-3 h-3"
...
export function RelationshipBadge({ stage, size = 'sm' }: Props) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 font-medium rounded-full ring-1 ${config.color} ${sizeClasses}`}
    >
      {config.icon}
      {config.label}
    </span>
