  ...
  return (
    <div className="flex items-center gap-4 px-6 py-2 border-b border-border-subtle bg-slate-1/50">
      ...
      <div className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-white/50">
        {' '}
        {/* typography-allow: impact-strip label */}
      ...
      <div className="flex items-center gap-1.5">
        ...
        {eviScore !== null ? (
          <>
            <span className="text-sm font-bold text-brand-cyan tabular-nums">
              {eviScore.toFixed(1)}
            </span>
            {eviDelta !== null && eviDelta !== 0 && (
              <span
                className={`text-xs ${eviDelta > 0 ? 'text-semantic-success' : 'text-semantic-danger'}`}
              >
                {eviDelta > 0 ? '↑' : '↓'}
                {Math.abs(eviDelta).toFixed(1)}
              </span>
            )}
          </>
        ) : (
