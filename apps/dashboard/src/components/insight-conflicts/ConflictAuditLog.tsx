      ...
      <div className="relative">
        ...
        <div className="space-y-0">
          {events.map((event, _index) => {
            const iconPath =
              EVENT_TYPE_ICONS[event.eventType] || EVENT_TYPE_ICONS.updated;
            const colorClass =
            ...
            return (
              <div key={event.id} className="relative pl-14 pr-4 py-4">
                {/* Icon */}
                <div
                  className={`absolute left-3 w-6 h-6 rounded-full ${colorClass} flex items-center justify-center`}
                >
                  <svg
                    className="w-3 h-3"
                    fill="none"
                  ...
                  {event.eventDetails &&
                    Object.keys(event.eventDetails).length > 0 && (
                      <div className="mt-2 text-xs text-gray-600 bg-gray-50 p-2 rounded">
                        {Object.entries(event.eventDetails).map(
                          ([key, value]) => (
                      ...
                      {event.previousState && (
                        <div className="flex-1 p-2 bg-red-50 rounded">
                          ...
                          <pre className="text-red-600 whitespace-pre-wrap break-words">
                            {JSON.stringify(event.previousState, null, 2).slice(
                              0,
                              200
                            )}
                          </pre>
                      ...
                      {event.newState && (
                        <div className="flex-1 p-2 bg-green-50 rounded">
                          ...
                          <pre className="text-green-600 whitespace-pre-wrap break-words">
                            {JSON.stringify(event.newState, null, 2).slice(
                              0,
                              200
                            )}
                          </pre>
