...
export function PRSituationBrief({
    ...
    relatedPillars: [
      {
        pillar: 'content',
        influence: 'informs',
        description: 'Coverage trends inform content strategy',
      },
      {
        pillar: 'exec',
        influence: 'updates',
        description: 'High-priority signals update executive digest',
      },
      {
        pillar: 'crisis',
        influence: 'affects',
        description: 'Negative signals may trigger crisis workflows',
      },
    ],
    confidence: 85,
    nextActions: [
      ...
      <div className="bg-panel border border-border-subtle rounded-xl p-6 shadow-lg shadow-slate-1/20">
        <div className="text-center py-12">
          ...
          <p className="text-sm text-slate-10 mt-1">
            Connect your media monitoring to get started
          </p>
          {onRefresh && (
            <button
              onClick={onRefresh}
  ...
  const criticalItems = data.attentionItems.filter(
    (i) => i.priority === 'critical'
  );
  const highRisks = data.signals.filter(
    ...
    <div className="bg-panel border border-border-subtle rounded-xl overflow-hidden shadow-lg shadow-slate-1/20">
      ...
      <div className="px-6 py-5 border-b border-border-subtle bg-gradient-to-r from-slate-3/50 to-slate-3/30">
        ...
        <div className="flex items-center gap-3 mt-5">
          ...
          >
            <span className="text-sm font-semibold text-white group-hover:text-brand-iris transition-colors">
              {data.stats.totalMentions}
            </span>
            <span className="text-sm text-slate-10 group-hover:text-slate-9 transition-colors">
              Mentions
            </span>
            <svg
              className="w-3 h-3 text-slate-10 group-hover:text-brand-iris transition-colors"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          >
            <span className="text-sm font-semibold text-semantic-success">
              {data.stats.positiveCoverage}
            </span>
            <span className="text-sm text-semantic-success/80 group-hover:text-semantic-success transition-colors">
              Positive
            </span>
            <svg
              className="w-3 h-3 text-semantic-success/60 group-hover:text-semantic-success transition-colors"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          >
            <span className="text-sm font-semibold text-semantic-danger">
              {data.stats.negativeCoverage}
            </span>
            <span className="text-sm text-semantic-danger/80 group-hover:text-semantic-danger transition-colors">
              Negative
            </span>
            <svg
              className="w-3 h-3 text-semantic-danger/60 group-hover:text-semantic-danger transition-colors"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          >
            <span className="text-sm font-semibold text-brand-iris">
              {data.stats.journalistsEngaged}
            </span>
            <span className="text-sm text-brand-iris/80 group-hover:text-brand-iris transition-colors">
              Journalists
            </span>
            <svg
              className="w-3 h-3 text-brand-iris/60 group-hover:text-brand-iris transition-colors"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div className="space-y-4">
            {data.attentionItems.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-slate-10 mt-1">
                  No items require immediate PR attention
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                          ...
                          {change.journalist && (
                            <span className="text-xs text-brand-iris">
                              {change.journalist}
                            </span>
                          )}
                          {change.sentiment && (
                            <span
                              className={`text-xs ${sentimentColors[change.sentiment]}`}
                            >
                              {change.sentiment.charAt(0).toUpperCase() +
                                change.sentiment.slice(1)}
                            </span>
                          )}
                          <span className="text-xs text-slate-10">
