...
const orchestrationStatusStyles: Record<
  string,
...
export function PRContinuityLinks({
  ...
  const buildReasoningContext = (): AIReasoningContext => ({
    ...
    triggerDescription:
      'PR Intelligence feeds signals to Content, SEO, Executive, and Crisis pillars for unified marketing intelligence',
    sourcePillar: 'pr',
    relatedPillars:
  ...
  const totalSignals = data.connections.reduce(
    (acc, c) => acc + c.signalCount,
    0
  );
      ...
      <div className="bg-panel border border-border-subtle rounded-xl p-5 shadow-lg shadow-slate-1/20">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs ${orchStatus.bg} ${orchStatus.text}`}
            >
              {orchStatus.icon}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${orchStatus.bg} ${orchStatus.text}`}
                >
                  {orchStatus.icon}
              ...
              <p className="text-sm text-slate-10 mt-0.5">
                {activeConnections.length} active connections · {totalSignals}{' '}
                signals flowing
              </p>
            </div>
          </div>
        ...
        {data.connections.map((conn) => {
          ...
          return (
            ...
            >
              ...
              {/* Description */}
              <p className="text-sm text-slate-11 mb-3 leading-relaxed">
                {conn.description}
              </p>
              ...
              {conn.latestSignal && (
                <div className="p-2.5 rounded-lg bg-slate-4/40 mb-3">
                  <p className="text-xs text-slate-11 line-clamp-2">
                    {conn.latestSignal}
                  </p>
                </div>
              )}
