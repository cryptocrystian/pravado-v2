    ...
    <div className={`${card.base} p-3`}>
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2">
          <div
            className={`p-1.5 rounded-md ${hasContent ? 'bg-brand-iris/10 text-brand-iris' : 'bg-slate-4 text-white/40'}`}
          >
            {config.icon}
          </div>
          <div>
            <h4
              className={`text-sm font-medium ${hasContent ? text.primary : text.muted}`}
            >
              {config.label}
            </h4>
        ...
        {/* Status Badge */}
        <span
          className={`px-2 py-0.5 text-xs font-medium rounded-full ${statusStyle.bg} ${statusStyle.text} border ${statusStyle.border}`}
        >
          {statusStyle.label}
        </span>
      </div>
...
export function DerivativesPanel({
  ...
  const handleCopy = useCallback(
    async (type: DerivativeType, content: string) => {
      try {
    ...
    <div className="p-4">
      ...
      {isBlocked && (
        <div className="mb-4 p-3 bg-semantic-danger/10 border border-semantic-danger/20 rounded-lg">
          <p className="text-xs text-semantic-danger font-medium">
            CiteMind has blocked this content. Derivative generation is disabled
      ...
      {isWarning && (
        <div className="mb-4 p-3 bg-semantic-warning/10 border border-semantic-warning/20 rounded-lg">
          <p className="text-xs text-semantic-warning">
            CiteMind has warnings. Derivatives can be generated but may require
            review.
          </p>
        </div>
      )}
      ...
      <div className="space-y-3">
        {DERIVATIVE_CONFIGS.map((config) => {
          ...
          const isStale =
            parentEdited || (derivative ? !derivative.valid : false);
          ...
          return (
            <DerivativeItem
              ...
              onCopy={() =>
                derivative && handleCopy(config.type, derivative.content)
              }
            />
          );
        })}
