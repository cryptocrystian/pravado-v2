...
export function ContextRailEditor({
  ...
  const { score: citeMindScore, gateStatus: realGateStatus } = useCiteMindScore(
    asset?.id
  );
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <p className="text-sm text-white/40 text-center">
            Select a document to see context
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3 min-h-0">
            ...
            citeMindScore
              ? {
                  ...
                  claim_verifiability_score:
                    citeMindScore.claim_verifiability_score,
                  structural_clarity_score:
                    citeMindScore.structural_clarity_score,
                  topical_authority_score:
                    citeMindScore.topical_authority_score,
        ...
        {(citeMindScore?.overall_score !== undefined ||
          contextData?.aeoScore !== undefined) && (
          <AEOScoreCard
            score={citeMindScore?.overall_score ?? contextData?.aeoScore ?? 0}
          />
        )}
          ...
          <section className="p-3 bg-slate-2 border border-slate-4 rounded-lg">
            ...
            <div className="flex flex-wrap gap-1.5">
              {contextData.entities.map((entity) => (
                <span
                  key={entity}
                  className="px-2 py-0.5 text-xs text-white/60 bg-white/5 rounded"
                >
                  {entity}
                </span>
              ))}
              ...
              {contextData.derivatives.map((d) => (
                ...
                >
                  <span className="text-white/50">
                    {DERIVATIVE_LABELS[d.type] || d.type.replace(/_/g, ' ')}
                  </span>
                  <span
                    className={
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-white/70">
                  {contextData.crossPillar.prHooks}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-white/50">SEO Hooks</span>
                <span className="font-medium text-white/70">
                  {contextData.crossPillar.seoHooks}
                </span>
              </div>
            </div>
          </section>
