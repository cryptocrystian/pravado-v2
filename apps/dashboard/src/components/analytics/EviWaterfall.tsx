  ...
  return (
    <div className="bg-slate-2 border border-white/10 rounded-lg p-2.5 text-xs shadow-lg">
      ...
      {d.isTotal ? (
        ...
        <p
          className={
            d.value >= 0 ? 'text-semantic-success' : 'text-semantic-danger'
          }
        >
          {d.value >= 0 ? '+' : ''}
          {d.value.toFixed(1)} pts
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="bg-panel border border-border-subtle rounded-xl p-5">
      <h3 className="text-xs font-semibold uppercase tracking-wider text-white/45 mb-4">
        ...
        <InfoTooltip
          content="Shows how each individual PR placement contributed to your overall EVI score. Taller bars mean that placement drove more AI visibility for your brand."
          size={11}
        />
      </h3>
      ...
      <div className="flex items-center gap-4 mt-2 text-[11px] text-white/40">
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-2 rounded-sm bg-[#00D9FF]/60 inline-block" />{' '}
          Total EVI
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-2 rounded-sm bg-[#22C55E] inline-block" />{' '}
          Positive lift
        </span>
      </div>
    </div>
