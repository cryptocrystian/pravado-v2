...
export function SEOPresenceReport({ orgName, period }: SEOPresenceReportProps) {
  ...
  const engines = Object.keys(mockEngineTrend[0] || {}).filter(
    (k) => k !== 'date'
  );
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 32 }}>
        {[
          { label: 'EVI Score', value: s.evi.value, delta: s.evi.delta },
          {
            ...
            value: s.shareOfVoice.value,
            delta: s.shareOfVoice.delta,
          },
          {
            label: 'Total Citations',
            value: s.totalCitations.value,
            delta: s.totalCitations.delta,
          },
          ...
          >
            ...
            {m.delta && (
              <p style={{ fontSize: 11, color: '#22C55E', margin: '2px 0 0' }}>
                {m.delta}
              </p>
            )}
          </div>
        ...
        {engines.map((engine) => (
          ...
          >
            ...
            <p style={{ fontSize: 20, fontWeight: 800, margin: 0 }}>
              {(latestEngines as unknown as Record<string, number>)[engine]}
            </p>
          </div>
        ))}
      </div>
