...
/**
 * AppSidebar - Legacy Sidebar Navigation
 *
 * Renders the sidebar navigation for routes that use the legacy shell.
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span
                className={`transition-colors duration-sm ${active ? 'text-brand-cyan' : 'text-white/55 group-hover:text-brand-cyan'}`}
              >
                {item.icon}
              </span>
              <span>{item.name}</span>
