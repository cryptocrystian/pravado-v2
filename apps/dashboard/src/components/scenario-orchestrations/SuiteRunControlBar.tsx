    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <span
            className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusBadgeClass(run.status)}`}
          >
          ...
          {canAdvance && (
            <>
              <button
                ...
                disabled={loading}
                className="px-4 py-2 text-sm font-medium text-white/90 bg-indigo-600 hover:bg-indigo-700 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {loading ? (
                  <>
                    <LoadingSpinner />
            ...
            (confirmAbort ? (
              <div className="flex items-center gap-2 ml-2">
                ...
                <button
                  onClick={handleAbort}
                  disabled={loading}
                  className="px-3 py-1.5 text-sm font-medium text-white/90 bg-red-600 hover:bg-red-700 rounded-lg disabled:opacity-50"
                >
      ...
      {isActive && run.totalItems && run.totalItems > 0 && (
        <div className="mt-4">
          <div className="flex justify-between text-xs text-gray-500 mb-1">
            <span>Progress</span>
            <span>
              {Math.round(((run.currentItemIndex || 0) / run.totalItems) * 100)}
              %
            </span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div
              ...
              style={{
                width: `${((run.currentItemIndex || 0) / run.totalItems) * 100}%`,
              }}
            />
          </div>
        </div>
        ...
        <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-red-700">
              ...
              <p className="mt-1">
                {typeof run.errorDetails === 'string'
                  ? run.errorDetails
