        ...
        {sortedItems.map((item, index) => {
          ...
          return (
            ...
            >
              {/* Status icon */}
              <div
                className={`relative z-10 flex items-center justify-center w-8 h-8 rounded-full border-2 ${iconClass}`}
              >
              ...
              <div className="flex-1 min-w-0 pb-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span
                    className={`px-1.5 py-0.5 rounded text-xs ${iconClass}`}
                  >
                    {SUITE_ITEM_STATUS_LABELS[item.status]}
                  </span>
                  {item.riskLevel && (
                    <span
                      className={`px-1.5 py-0.5 rounded text-xs ${getRiskBadgeClass(item.riskLevel)}`}
                    >
                      {item.riskLevel}
                    </span>
                  )}
