...
function StatusIcon({ status }: { status: string }) {
  ...
  if (status === 'published') {
    return (
      <svg
        className="w-3.5 h-3.5 text-semantic-success shrink-0"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
  ...
  if (status === 'failed') {
    return (
      <svg
        className="w-3.5 h-3.5 text-semantic-danger shrink-0"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
  ...
  if (compact) {
    ...
    return (
      ...
      >
        ...
        <span
          className={`w-1.5 h-1.5 rounded-full shrink-0 ${pillarConf?.dotClass ?? ''}`}
        />
      </button>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        {/* Pillar badge */}
        <span
          className={`px-1.5 py-0.5 text-[11px] font-bold uppercase tracking-wider rounded border ${pillarConf?.badgeClass ?? ''}`}
        >
        ...
        {/* Mode badge */}
        <span
          className={`px-1.5 py-0.5 text-[11px] font-bold uppercase tracking-wider rounded border ${modeConf?.badgeClass ?? ''}`}
        >
        ...
        {riskConf && riskConf.dotClass && (
          <span
            ...
            title={`Risk: ${riskConf.label}`}
          />
        )}

        {/* Status badge (right-aligned) */}
        <span
          className={`ml-auto px-1.5 py-0.5 text-[11px] font-bold uppercase tracking-wider rounded border ${statusConf?.badgeClass ?? ''}`}
        >
      ...
      {/* Summary */}
      <p className="text-[13px] text-white/55 mt-1 line-clamp-2 leading-relaxed">
      ...
      {item.details.dependencies.length > 0 && (
        <div className="flex items-center gap-1.5 mt-2">
          ...
          <span className="text-[11px] font-bold uppercase tracking-wider text-white/40">
            {item.details.dependencies.length} dep
            {item.details.dependencies.length !== 1 ? 's' : ''}
          </span>
        </div>
      )}
    </button>
