...
const pillarColors: Record<
  Pillar,
  { text: string; bg: string; bgHover: string }
> = {
  pr: {
  ...
  return (
    <div className="action-hover-brief-v5 space-y-3 max-h-[320px] overflow-y-auto">
      ...
      <div className="pb-2 border-b border-white/10">
        <h4
          className={`text-sm font-semibold ${pillarStyle.text} line-clamp-2 leading-snug`}
        >
          {action.title}
        </h4>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {action.signals.slice(0, 3).map((signal, idx) => (
              ...
              >
                ...
                <span
                  className={`text-xs font-bold ${signalToneColors[signal.tone]}`}
                >
                  {signal.value}
                </span>
              </div>
      ...
      {action.guardrails && action.guardrails.length > 0 && (
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5">
            <svg
              className="w-3.5 h-3.5 text-semantic-warning/70"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <span className="text-[11px] font-bold uppercase tracking-wider text-semantic-warning/70">
              Guardrails
            </span>
          </div>
          <div className="space-y-1">
