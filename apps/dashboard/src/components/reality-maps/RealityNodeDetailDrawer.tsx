...
export function RealityNodeDetailDrawer({
  ...
  const nodeType =
    'nodeType' in displayNode
      ? displayNode.nodeType
      : (displayNode as RealityMapGraphNode).type;
  const summary =
    'aiSummary' in displayNode
      ? displayNode.aiSummary
      : (displayNode as RealityMapGraphNode).summary;
  const keyDrivers: KeyDriver[] =
    'keyDrivers' in displayNode ? displayNode.keyDrivers : [];
  const riskFactors: RealityMapRiskFactor[] =
    'riskFactors' in displayNode ? displayNode.riskFactors : [];
  const opportunityFactors: OpportunityFactor[] =
    'opportunityFactors' in displayNode ? displayNode.opportunityFactors : [];
    ...
    <div className="fixed inset-y-0 right-0 w-96 bg-white shadow-xl z-50 overflow-hidden flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
        <div className="flex items-center gap-2">
          <span
            className={`px-2 py-0.5 rounded text-xs font-medium ${getNodeTypeBadgeClass(nodeType)}`}
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="flex gap-2">
          <span
            className={`px-2 py-1 rounded text-xs font-medium ${getRiskLevelBadgeClass(getRiskLevel(node.riskScore))}`}
          >
            Risk: {getRiskLevel(node.riskScore)}
          </span>
          <span
            className={`px-2 py-1 rounded text-xs font-medium ${getOpportunityLevelBadgeClass(getOpportunityLevel(node.opportunityScore))}`}
          >
              ...
              {riskFactors.map((risk, idx) => (
                <div key={idx} className="p-2 bg-red-50 rounded-lg">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span
                      className={`text-xs px-1.5 py-0.5 rounded ${getRiskLevelBadgeClass(risk.severity || 'medium')}`}
                    >
                      {risk.severity}
                    </span>
                  </div>
              ...
              {opportunityFactors.map((opp, idx) => (
                <div key={idx} className="p-2 bg-green-50 rounded-lg">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span
                      className={`text-xs px-1.5 py-0.5 rounded ${getOpportunityLevelBadgeClass(opp.potential || 'medium')}`}
                    >
                      {opp.potential}
                    </span>
                  </div>
            ...
            {'cumulativeProbability' in node && (
              <>
                ...
                <div className="text-gray-900">
                  {formatProbability(node.cumulativeProbability)}
                </div>
              </>
            )}
            {node.parentId && (
              <>
                ...
                <div className="text-gray-900 font-mono truncate">
