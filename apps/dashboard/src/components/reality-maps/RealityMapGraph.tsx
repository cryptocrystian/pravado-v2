...
export function RealityMapGraph({
  ...
  const isNodeHighlighted = useCallback(
    (nodeId: string) => {
      ...
      const path = graphData.paths.find((p) =>
        highlightedPathIds.includes(p.id)
      );
      ...
      <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg border border-gray-200">
        <div className="text-center text-gray-500">
          ...
          <p className="text-sm mt-1">
            Generate the reality map to see the visualization
          </p>
        </div>
      </div>
    );
          ...
          {nodes.map((node) => {
            ...
            return (
              ...
              >
                ...
                {/* Node type indicator */}
                {node.type === 'root' && (
                  <circle r={size / 4} fill="white" opacity={0.5} />
                )}
                ...
                {isHovered && (
                  <g>
                    ...
                    <text
                      ...
                      fill="#6B7280"
                    >
                      Risk: {formatScore(node.riskScore)} | Opp:{' '}
