...
const CHANNELS: ReputationAlertChannel[] = [
  'in_app',
  'email',
  'slack',
  'webhook',
];
const COMPONENTS: ReputationComponentKey[] = [
  'sentiment',
  'coverage',
  'crisis_impact',
...
export function AlertRuleForm({ rule, onSave, onCancel }: AlertRuleFormProps) {
  const [formData, setFormData] = useState<
    Partial<CreateReputationAlertRuleInput>
  >({
    name: '',
    description: '',
    isActive: true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (rule) {
        ...
        savedRule = await createAlertRule(
          formData as CreateReputationAlertRuleInput
        );
      }
  ...
  const updateField = (
    field: keyof CreateReputationAlertRuleInput,
    value: unknown
  ) => {
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                updateField('componentKey', e.target.value || undefined)
              }
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            >
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                updateField('competitorSlug', e.target.value || undefined)
              }
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center">
            <input
              ...
              onChange={(e) =>
                updateField('linkCrisisIncidents', e.target.checked)
              }
              className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
            />
            <label className="ml-2 block text-sm text-gray-900">
              Link Crisis Incidents
            </label>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">
              Min Incident Severity
            </label>
            <input
              type="number"
              min="1"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                updateField('timeWindowMinutes', Number(e.target.value))
              }
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
            <p className="mt-1 text-xs text-gray-500">
              Evaluation window for delta calculations
            </p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">
