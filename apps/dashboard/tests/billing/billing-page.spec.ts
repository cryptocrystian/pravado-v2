...
test.describe('Billing Page', () => {
  test.beforeEach(async ({ page }) => {
    // Mock API responses
    await page.route(
      '**/api/v1/billing/org/summary-enriched',
      async (route) => {
        await route.fulfill({
          status: 200,
  ...
  test('should display billing page with current plan', async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Manage Payment Method")')
    ).toBeVisible();
  });
  ...
  test('should display usage progress bars', async ({ page }) => {
    ...
    await expect(
      page.locator('h2:has-text("Usage This Period")')
    ).toBeVisible();
  ...
  test('should display available plans grid', async ({ page }) => {
    ...
    const starterCard = page
      .locator('.text-xl:has-text("Starter")')
      .locator('..')
      .locator('..');
    await expect(
      starterCard.locator('button:has-text("Current Plan")')
    ).toBeVisible();
  });
  ...
  test('should show cancel subscription modal', async ({ page }) => {
    ...
    // Check modal appears
    await expect(
      page.locator('h3:has-text("Cancel Subscription")')
    ).toBeVisible();
    await expect(
    ...
    await expect(
      page.locator('text=Cancel at period end (Recommended)')
    ).toBeVisible();
    await expect(page.locator('text=Cancel immediately')).toBeVisible();

    // Check buttons
    await expect(
      page.locator('button:has-text("Keep Subscription")')
    ).toBeVisible();
    await expect(
      page.locator('button:has-text("Confirm Cancel")')
    ).toBeVisible();
  ...
  test('should handle downgrade with usage guardrails', async ({ page }) => {
    ...
    const internalDevCard = page
      .locator('.text-xl:has-text("Internal Dev")')
      .locator('..')
      .locator('..');
    ...
    await expect(
      page.locator('h3:has-text("Cannot Downgrade Plan")')
    ).toBeVisible();
  ...
  test('should show Enterprise contact sales button', async ({ page }) => {
    ...
    const enterpriseCard = page
      .locator('.text-xl:has-text("Enterprise")')
      .locator('..')
      .locator('..');

    // Check it has Contact Sales button
    await expect(
      enterpriseCard.locator('button:has-text("Contact Sales")')
    ).toBeVisible();
  });
  ...
  test('should show billing history CTA', async ({ page }) => {
    ...
    await expect(
      page.locator('text=View past invoices and payment history')
    ).toBeVisible();
    await expect(
      page.locator('button:has-text("View Invoices (Coming in S34)")')
    ).toBeDisabled();
  });

  test('should display overage breakdown when overages exist', async ({
    page,
  }) => {
