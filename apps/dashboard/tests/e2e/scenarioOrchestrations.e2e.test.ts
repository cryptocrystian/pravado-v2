...
const mockRunItems = [
  {
    ...
    riskLevel: 'high',
    keyFindings: [
      'Crisis detected in social media',
...
const mockFetch = (url: string, options?: RequestInit) => {
  ...
  // List suites
  if (
    path === '/api/v1/scenario-orchestrations/suites' &&
    (!options || options.method === 'GET' || !options.method)
  ) {
    return Promise.resolve({
      ok: true,
      json: () =>
        Promise.resolve({ suites: mockSuites, total: mockSuites.length }),
    });
  }

  // Get suite
  if (
    path.match(/\/api\/v1\/scenario-orchestrations\/suites\/suite-\d+$/) &&
    (!options || options.method === 'GET' || !options.method)
  ) {
  ...
  if (
    path.match(
      /\/api\/v1\/scenario-orchestrations\/suites\/suite-\d+\/runs$/
    ) &&
    options?.method === 'POST'
  ) {
    return Promise.resolve({
  ...
  // Get run
  if (
    path.match(/\/api\/v1\/scenario-orchestrations\/suite-runs\/run-\d+$/) &&
    (!options || options.method === 'GET' || !options.method)
  ) {
    return Promise.resolve({
      ok: true,
  ...
  if (
    path.match(
      /\/api\/v1\/scenario-orchestrations\/suite-runs\/run-\d+\/items$/
    ) &&
    (!options || options.method === 'GET' || !options.method)
  ) {
    return Promise.resolve({
  ...
  if (
    path.match(
      /\/api\/v1\/scenario-orchestrations\/suite-runs\/run-\d+\/advance$/
    ) &&
    options?.method === 'POST'
  ) {
    return Promise.resolve({
  ...
  if (
    path.match(
      /\/api\/v1\/scenario-orchestrations\/suite-runs\/run-\d+\/abort$/
    ) &&
    options?.method === 'POST'
  ) {
    return Promise.resolve({
  ...
  if (
    path.match(
      /\/api\/v1\/scenario-orchestrations\/suite-runs\/run-\d+\/narrative$/
    ) &&
    options?.method === 'POST'
  ) {
    return Promise.resolve({
      ...
      json: () =>
        Promise.resolve({
          success: true,
          narrative:
            'The crisis simulation revealed significant vulnerabilities in the initial response...',
        }),
    });
  }
  ...
  if (
    path.match(
      /\/api\/v1\/scenario-orchestrations\/suite-runs\/run-\d+\/risk-map$/
    ) &&
    options?.method === 'POST'
  ) {
    return Promise.resolve({
            ...
            risks: [
              {
                category: 'Reputation',
                level: 'high',
                description: 'Negative coverage risk',
                mitigation: 'Prepare holding statements',
              },
              {
                category: 'Operational',
                level: 'medium',
                description: 'Response delay risk',
                mitigation: 'Pre-draft communications',
              },
            ],
            opportunities: [
              {
                category: 'Brand',
                description: 'Demonstrate transparency',
                potential: 'Trust building',
              },
            ],
          },
        }),
  ...
  if (
    path.match(
      /\/api\/v1\/scenario-orchestrations\/suites\/suite-\d+\/archive$/
    ) &&
    options?.method === 'POST'
  ) {
    return Promise.resolve({
...
describe('Scenario Orchestrations E2E', () => {
  ...
  describe('Suite List View', () => {
    it('should load and display suites', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suites'
      );
    ...
    it('should display stats correctly', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/stats'
      );
      const data = await response.json();
    ...
    it('should filter suites by status', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suites?status=configured'
      );
      const data = await response.json();
  ...
  describe('Suite Detail View', () => {
    it('should load suite with items', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suites/suite-1'
      );
      const data = await response.json();
    ...
    it('should display trigger conditions for items', async () => {
      ...
      const escalationItem = data.items.find(
        (i: (typeof mockSuiteItems)[0]) => i.label === 'Escalation Response'
      );
  ...
  describe('Suite Run Execution', () => {
    it('should start a suite run', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suites/suite-1/runs',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({}),
    ...
    it('should get run details', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suite-runs/run-1'
      );
      const data = await response.json();
    ...
    it('should get run items', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suite-runs/run-1/items'
      );
      const data = await response.json();
    ...
    it('should advance to next item', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suite-runs/run-1/advance',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({}),
    ...
    it('should abort a running run', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suite-runs/run-1/abort',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  describe('Narrative Generation', () => {
    it('should generate suite narrative', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suite-runs/run-1/narrative',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  describe('Risk Map Generation', () => {
    it('should generate suite risk map', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suite-runs/run-1/risk-map',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  describe('Suite Archival', () => {
    it('should archive a suite', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suites/suite-1/archive',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  describe('Condition Evaluation Display', () => {
    it('should display condition evaluation results', async () => {
      ...
      const secondItem = data.items.find(
        (i: (typeof mockRunItems)[0]) => i.orderIndex === 1
      );
  ...
  describe('Timeline Visualization', () => {
    it('should have items in correct order', async () => {
      ...
      const sortedItems = [...data.items].sort(
        (a: (typeof mockRunItems)[0], b: (typeof mockRunItems)[0]) =>
    ...
    it('should show completed items with metrics', async () => {
      ...
      const completedItem = data.items.find(
        (i: (typeof mockRunItems)[0]) => i.status === 'completed'
      );
  ...
  describe('Metrics Aggregation', () => {
    it('should aggregate metrics from run items', async () => {
      ...
      const totalSteps = data.items.reduce(
        (sum: number, item: (typeof mockRunItems)[0]) =>
          sum + (item.stepsExecuted || 0),
        0
      );
      const totalTokens = data.items.reduce(
        (sum: number, item: (typeof mockRunItems)[0]) =>
          sum + (item.tokensUsed || 0),
        0
      );
  ...
  describe('Error Handling', () => {
    it('should handle 404 for non-existent suite', async () => {
      const response = await fetch(
        'http://localhost:4000/api/v1/scenario-orchestrations/suites/non-existent'
      );
