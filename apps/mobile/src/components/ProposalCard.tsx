...
export function ProposalCard({ proposal, onApprove, onDismiss }: Props) {
  ...
  return (
    <View style={[s.card, { borderLeftColor: pillarColor }]}>
      <View style={s.top}>
        <View
          style={[
            ...
            {
              backgroundColor:
                (PRIORITY_COLORS[proposal.priority] || colors.textDim) + '18',
            },
          ]}
        >
          <Text
      ...
      <Text style={s.rationale} numberOfLines={2}>
        {proposal.rationale}
      </Text>
      <View style={s.footer}>
        <View style={s.eviChip}>
          <Text style={s.eviText}>
            +{proposal.evi_impact_estimate.toFixed(1)} EVI
          </Text>
        </View>
