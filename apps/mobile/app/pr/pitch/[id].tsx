    ...
    return (
      <ScrollView style={st.container} contentContainerStyle={st.content}>
        <EmptyState
          ...
          subtitle="This pitch may have been deleted"
        />
      </ScrollView>
    );
  }
      ...
      <ScrollView contentContainerStyle={st.content}>
        ...
        <View
          style={[
            ...
            {
              backgroundColor:
                (STATUS_COLORS[pitch.status] || colors.textDim) + '18',
            },
          ]}
        >
          <Text
            style={[
              st.statusText,
              { color: STATUS_COLORS[pitch.status] || colors.textDim },
            ]}
          >
        ...
        <View style={st.card}>
          <View style={st.journRow}>
            <View style={st.avatar}>
              <Text style={st.avatarText}>{pitch.journalist_name[0]}</Text>
            </View>
            <View style={{ flex: 1 }}>
              ...
              <View key={step} style={st.timelineRow}>
                <View style={st.timelineLeft}>
                  <View
                    style={[
                      st.dot,
                      {
                        backgroundColor: reached ? colors.green : colors.border,
                      },
                    ]}
                  />
                ...
                <View style={st.timelineContent}>
                  <Text
                    style={[
                      st.stepLabel,
                      { color: reached ? colors.textPrimary : colors.textDim },
                    ]}
                  >
                    {step}
                  </Text>
                  {ts && (
                    <Text style={st.stepDate}>
      ...
      <View style={st.actionsBar}>
        {pitch.status === 'draft' && (
          <TouchableOpacity
            style={st.editBtn}
            onPress={() =>
              Linking.openURL(`https://app.pravado.io/app/pr/pitches/${id}`)
            }
          >
