  ...
  return (
    ...
    >
      ...
      <View style={s.card}>
        {evi ? (
          ...
          <EmptyState
            icon="analytics"
            title="No EVI data"
            subtitle="EVI will appear after first calculation"
          />
        )}
      </View>
          ...
          {evi.drivers.map((d) => (
            <View key={d.type} style={s.driverCard}>
              ...
              <Text
                style={[
                  s.driverDelta,
                  { color: d.delta_7d >= 0 ? colors.green : colors.red },
                ]}
              >
                {d.delta_7d >= 0 ? '+' : ''}
                {d.delta_7d.toFixed(1)}
              </Text>
            </View>
          ))}
        </View>
      ...
      <EmptyState
        icon="trophy"
        title="Top Wins"
        subtitle="Your wins will appear as SAGE tracks visibility improvements"
      />
    </ScrollView>
  );
}
