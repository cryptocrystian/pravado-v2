...
const STATUS_COLORS: Record<string, string> = {
  ...
  sent: colors.cyberBlue,
  opened: colors.yellow,
    ...
    <ScrollView
      ...
      refreshControl={
        <RefreshControl
          ...
          onRefresh={onRefresh}
          tintColor={colors.electricPurple}
        />
      }
    >
      <View style={s.tabs}>
      ...
      {tab === 'Pitches' &&
        (pitches.length === 0 ? (
          <EmptyState
            ...
            subtitle="Pitches you create will appear here"
          />
        ) : (
          pitches.map((p) => (
            <TouchableOpacity
              ...
              style={[
                ...
                {
                  borderLeftWidth: 3,
                  borderLeftColor: STATUS_COLORS[p.status] || colors.textDim,
                },
              ]}
              onPress={() => router.push(`/pr/pitch/${p.id}`)}
            >
              <Text style={s.pitchName}>
                {p.journalist_name} · {p.outlet_name}
              </Text>
              <Text style={s.pitchSubject} numberOfLines={1}>
                {p.subject}
              </Text>
              <View style={s.pitchMeta}>
                <Text
                  style={[s.statusBadge, { color: STATUS_COLORS[p.status] }]}
                >
                  {p.status.toUpperCase()}
                </Text>
                {p.sent_at && (
                  <Text style={s.pitchDate}>
      ...
      {tab === 'Journalists' &&
        (journalists.length === 0 ? (
          <EmptyState
            ...
            subtitle="Journalist contacts will appear here"
          />
        ) : (
          journalists.map((j) => (
            <View key={j.id} style={s.card}>
              <View style={s.journRow}>
                <View style={s.journAvatar}>
                  <Text style={s.journInitial}>{j.name[0]}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={s.journName}>{j.name}</Text>
                  <Text style={s.journOutlet}>
                    {j.outlet_name} · {j.beat}
                  </Text>
                </View>
                <View
                  style={[
