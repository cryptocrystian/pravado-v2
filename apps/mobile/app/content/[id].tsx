...
const GATE_MSG: Record<string, string> = {
  ...
  blocked:
    'Content does not meet quality standards — needs improvement before publishing.',
};
    ...
    <View style={st.container}>
              ...
              {FACTORS.map((f) => {
                ...
                return (
                  <View key={f.key} style={st.factorRow}>
                    ...
                    <Text style={[st.factorVal, { color: barColor(val) }]}>
                      {Math.round(val)}
                    </Text>
                  </View>
                );
        ...
        {bodyPreview && (
          <View style={st.card}>
            ...
            <Text style={st.bodyText}>
              {bodyPreview}
              {!expanded && item.body && item.body.length > 800 ? '...' : ''}
            </Text>
            {item.body && item.body.length > 800 && (
              <TouchableOpacity onPress={() => setExpanded(!expanded)}>
                <Text style={st.expandLink}>
      ...
      <View style={st.actionsBar}>
        ...
        <TouchableOpacity
          style={st.openBtn}
          onPress={() =>
            Linking.openURL(`https://app.pravado.io/app/content/${id}`)
          }
        >
