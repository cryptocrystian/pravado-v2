...
function createMock(responders: Record<string, () => { data: any; error: any }>) {
  function builder(table: string) {
    ...
    b.upsert = vi.fn(() => { op = 'upsert'; return b; });
  ...
  return { from: vi.fn((t: string) => builder(t)), rpc: vi.fn(async () => ({ data: null, error: null })) } as unknown as SupabaseClient;
}
...
const ctx = (overrides = {}) => ({
  ...
  personalization: { name: 'Sarah', outlet: 'TechCrunch', beats: [], recentWorkHook: null },
  ...overrides,
});
...
describe('createSupabaseGovernanceGateways.getContactGovernanceState', () => {
  it('H1: FAILS CLOSED when media_contacts read returns an error', async () => {
    const supabase = createMock({
      ...
      'media_contacts:select': () => ({ data: null, error: { message: 'boom', code: '500' } }),
    });
  ...
  it('H1: FAILS CLOSED when contact_emails read errors', async () => {
    const supabase = createMock({
      'suppressed_email_hashes:select': () => ({ data: null, error: null }),
      'contact_emails:select': () => ({ data: null, error: { message: 'boom' } }),
    });
    const g = createSupabaseGovernanceGateways(supabase);
  ...
  it('H1: FAILS CLOSED when the suppression hash read errors', async () => {
    const supabase = createMock({
      'suppressed_email_hashes:select': () => ({ data: null, error: { message: 'boom' } }),
    });
    const g = createSupabaseGovernanceGateways(supabase);
    const state = await g.getContactGovernanceState(ctx());
    expect(state.readError).toBe(true);
  ...
  it('H1b: a hashed opt-out blocks even with NO contact row (pre-backfill)', async () => {
    ...
    const g = createSupabaseGovernanceGateways(supabase);
    const state = await g.getContactGovernanceState(ctx({ contactId: null, journalistId: null }));
    expect(state.state).toBe('suppressed');
  });
});

describe('end-to-end: gateway + chokepoint', () => {
  const rawSend = vi.fn(async (): Promise<SendEmailResponse> => ({ success: true, messageId: 'm', provider: 'stub' }));
