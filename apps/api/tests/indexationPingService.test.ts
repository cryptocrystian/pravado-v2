...
describe('submitIndexNow', () => {
  it('POSTs host/key/urlList to the IndexNow endpoint and reports success on 200', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(fakeResponse(200)) as unknown as FetchLike;
  ...
  it('does not throw on network failure', async () => {
    const fetchMock = vi
      .fn()
      .mockRejectedValue(new Error('boom')) as unknown as FetchLike;
    const res = await submitIndexNow(
      ['https://acme.example/a'],
      { key: 'k' },
      fetchMock
...
describe('pingGoogleIndexing', () => {
  it('POSTs URL_UPDATED with a Bearer token', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(fakeResponse(200)) as unknown as FetchLike;
    const res = await pingGoogleIndexing(
      'https://acme.example/a',
      'tok-123',
      fetchMock
    );
...
describe('getGoogleIndexingAccessToken', () => {
  ...
  it('signs a JWT (RS256) and exchanges it for an access token', async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        fakeResponse(200, { access_token: 'ya29.test' })
      ) as unknown as FetchLike;

    const token = await getGoogleIndexingAccessToken(
      {
        email: 'svc@acme.iam.gserviceaccount.com',
...
describe('pingIndexationOnPublish', () => {
  function supa(): SupabaseClient {
    return createMockSupabaseClient({
      indexation_pings: { data: null, error: null },
    });
  }
