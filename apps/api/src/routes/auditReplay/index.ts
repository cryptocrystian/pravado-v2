 ...
 */
export async function auditReplayRoutes(
    ...
    async (request, reply) => {
      ...
      if (job.status === 'success' || job.status === 'failed') {
        reply.raw.write(
          `data: ${JSON.stringify({
            type:
              job.status === 'success' ? 'replay.completed' : 'replay.failed',
            data: {
              runId: id,
              result: job.result,
  ...
  server.get<{
    ...
    '/api/v1/audit/replay/:id/snapshots/:index',
    { preHandler: requireUser },
    async (request, reply) => {
      try {
  ...
  server.get<{
    ...
    async (request, reply) => {
      try {
        ...
        const { runs, total } = await replayService.listReplayJobs(
          orgId,
          limit,
          offset
        );
