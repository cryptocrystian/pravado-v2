...
export async function onboardingRoutes(server: FastifyInstance) {
  ...
  }>('/brand', { preHandler: requireUser }, async (request, reply) => {
  ...
  // ========================================================================
  server.get('/status', { preHandler: requireUser }, async (request, reply) => {
    const userId = request.user!.id;
    ...
    const { data: org } = await supabase
      .from('orgs')
      .select(
        'id, name, domain, industry, company_size, onboarding_step, completed_onboarding_at, onboarding_skips'
      )
      .eq('id', orgId)
      .single();
    ...
    async (request, reply) => {
      ...
      if (!orgId) {
        return reply.code(404).send({
          ...
          error: {
            code: 'NO_ORG',
            message: 'No organization found. Complete brand setup first.',
          },
        });
      }
      ...
      if (skipped) {
        ...
        const currentSkips =
          (org?.onboarding_skips as Record<string, boolean>) ?? {};
        updateData.onboarding_skips = {
          ...currentSkips,
          [`step_${step}`]: true,
        };
      }
  ...
  // ========================================================================
  server.post<{
    Body: { competitors: Array<{ domain: string; name?: string }> };
  }>('/competitors', { preHandler: requireUser }, async (request, reply) => {
    const userId = request.user!.id;
    const orgId = await getUserOrgId(userId);
  ...
  server.post<{
  ...
  }>('/journalists', { preHandler: requireUser }, async (request, reply) => {
    const userId = request.user!.id;
    const orgId = await getUserOrgId(userId);
    ...
    if (!Array.isArray(journalists)) {
      return reply.code(400).send({
        ...
        error: {
          code: 'VALIDATION_ERROR',
          message: 'journalists must be an array',
        },
      });
    }
  ...
  server.post<{
    Body: { urls: string[] };
  }>('/content', { preHandler: requireUser }, async (request, reply) => {
    const userId = request.user!.id;
    const orgId = await getUserOrgId(userId);
