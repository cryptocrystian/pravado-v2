...
function getUserId(request: FastifyRequest): string {
  const userId =
    (request as any).user?.id || (request.headers['x-user-id'] as string);
  if (!userId) {
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const incident = await service.createIncident(
          orgId,
          userId,
          validated as CreateIncidentRequest
        );
      ...
      try {
        ...
        const filters = listIncidentsSchema.parse({
          ...
          trajectory: query.trajectory
            ? query.trajectory.split(',')
            : undefined,
          propagationLevel: query.propagationLevel
            ? query.propagationLevel.split(',')
            : undefined,
          ...
          isEscalated:
            query.isEscalated === 'true'
              ? true
              : query.isEscalated === 'false'
                ? false
                : undefined,
          escalationLevelGte: query.escalationLevelGte
            ? parseInt(query.escalationLevelGte, 10)
            : undefined,
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const incident = await service.updateIncident(
          ...
          userId,
          id,
          validated as UpdateIncidentRequest
        );
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const incident = await service.closeIncident(
          orgId,
          userId,
          id,
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const incident = await service.escalateIncident(
          orgId,
          userId,
          id,
          level
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const userId = getUserId(request);
        const { id } = request.params as { id: string };
        const actions = await service.generateActionRecommendations(
          orgId,
          userId,
          id
        );
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
          ...
          isEscalated:
            query.isEscalated === 'true'
              ...
              : query.isEscalated === 'false'
                ? false
                : undefined,
          sourceSystems: query.sourceSystems
            ? query.sourceSystems.split(',')
            : undefined,
        ...
        const result = await service.getSignals(
          ...
          {
            ...filters,
            windowFrom: filters.windowFrom
              ? new Date(filters.windowFrom)
              : undefined,
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const body = request.body as
          | { linkedIncidentId?: string; resolutionNotes?: string }
          | undefined;
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const result = await service.runDetection(orgId, userId, {
          ...
          sourceSystems: validated.sourceSystems as
            | CrisisSourceSystem[]
            | undefined,
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const result = await service.generateCrisisBrief(
          ...
          userId,
          id,
          validated as GenerateCrisisBriefRequest
        );
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const result = await service.getBriefs(
          orgId,
          filters as BriefFilters,
          filters.limit || 20,
          filters.offset || 0
        );
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const orgId = getOrgId(request);
        const userId = getUserId(request);
        const { briefId, sectionId } = request.params as {
          briefId: string;
          sectionId: string;
        };
        const body = request.body as Record<string, unknown> | undefined;
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const rule = await service.createEscalationRule(
          orgId,
          userId,
          validated as CreateEscalationRuleRequest
        );
  ...
  server.patch(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const rule = await service.updateEscalationRule(
          orgId,
          userId,
          id,
          validated as UpdateEscalationRuleRequest
        );
