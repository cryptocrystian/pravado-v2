...
export async function gscRoutes(server: FastifyInstance) {
  ...
  server.get('/auth-url', async (request, reply) => {
    if (!FLAGS.ENABLE_GSC_INTEGRATION) {
      return reply.code(404).send({
        ...
        error: {
          ...
          message: 'GSC integration not enabled',
        },
      });
    }
    ...
    const state = Buffer.from(
      JSON.stringify({ orgId, userId: (request as any).user.id })
    ).toString('base64');
  ...
  server.get<{
    ...
    if (oauthError) {
      ...
      return reply.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || ''}/app/seo?gsc=error&reason=${oauthError}`
      );
    }

    if (!code || !state) {
      return reply.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || ''}/app/seo?gsc=error&reason=missing_params`
      );
    }
    ...
    try {
      ...
      if (!orgId) {
        return reply.redirect(
          `${process.env.NEXT_PUBLIC_APP_URL || ''}/app/seo?gsc=error&reason=invalid_state`
        );
      }
      ...
      if (!tokenRes.ok) {
        ...
        return reply.redirect(
          `${appUrl}/app/seo?gsc=error&reason=token_exchange`
        );
      }
      ...
      // Fetch user email
      const { fetchGoogleUserEmail, fetchGscSites } = await import(
        '../../services/gsc/gscSyncService'
      );
      ...
      return reply.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || ''}/app/seo?gsc=error&reason=server_error`
      );
    }
  });
