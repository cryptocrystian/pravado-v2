...
export default async function journalistEnrichmentRoutes(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const record = await service.createRecord(
          orgId,
          input as Parameters<typeof service.createRecord>[1],
          userId
        );
      ...
      try {
        ...
        const query = EnrichmentRecordsQuerySchema.parse({
          sourceTypes: queryParams.sourceTypes
            ? Array.isArray(queryParams.sourceTypes)
          ...
          status: queryParams.status
            ? Array.isArray(queryParams.status)
              ...
              : [queryParams.status]
            : undefined,
          minConfidenceScore: queryParams.minConfidenceScore
            ? Number(queryParams.minConfidenceScore)
            : undefined,
          maxConfidenceScore: queryParams.maxConfidenceScore
            ? Number(queryParams.maxConfidenceScore)
            : undefined,
          minCompletenessScore: queryParams.minCompletenessScore
            ? Number(queryParams.minCompletenessScore)
            : undefined,
          emailVerified:
            queryParams.emailVerified === 'true'
              ...
              : queryParams.emailVerified === 'false'
                ...
                : undefined,
          hasEmail:
            queryParams.hasEmail === 'true'
              ? true
              : queryParams.hasEmail === 'false'
                ? false
                : undefined,
          hasPhone:
            queryParams.hasPhone === 'true'
              ? true
              : queryParams.hasPhone === 'false'
                ? false
                : undefined,
          hasSocialProfiles:
            queryParams.hasSocialProfiles === 'true'
              ? true
              : queryParams.hasSocialProfiles === 'false'
                ? false
                : undefined,
          outlet: queryParams.outlet as string | undefined,
          qualityFlags: queryParams.qualityFlags
            ? Array.isArray(queryParams.qualityFlags)
              ? queryParams.qualityFlags
              : [queryParams.qualityFlags]
            : undefined,
          hasPotentialDuplicates:
            queryParams.hasPotentialDuplicates === 'true'
              ? true
              : queryParams.hasPotentialDuplicates === 'false'
                ? false
                : undefined,
        ...
        return reply.status(500).send({
          error:
            error instanceof Error
              ? error.message
              : 'Failed to list enrichment records',
        });
      }
    }
  );
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : 'Failed to get enrichment record';
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const record = await service.updateRecord(
          orgId,
          id,
          input as Parameters<typeof service.updateRecord>[2]
        );
        ...
        const message =
          error instanceof Error
            ? error.message
            : 'Failed to update enrichment record';
        return reply.status(message.includes('not found') ? 404 : 500).send({
          error: message,
        });
      }
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : 'Failed to delete enrichment record';
        return reply.status(message.includes('not found') ? 404 : 500).send({
          error: message,
        });
      }
    ...
    try {
      ...
      const query = EnrichmentJobsQuerySchema.parse({
        jobType: queryParams.jobType
          ? Array.isArray(queryParams.jobType)
            ? queryParams.jobType
            : [queryParams.jobType]
          : undefined,
        status: queryParams.status
        ...
        minProgressPercentage: queryParams.minProgressPercentage
          ? Number(queryParams.minProgressPercentage)
          : undefined,
        sortBy: queryParams.sortBy as string | undefined,
        sortOrder: queryParams.sortOrder as string | undefined,
        limit: queryParams.limit ? Number(queryParams.limit) : undefined,
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : 'Failed to get merge suggestions';
        return reply.status(message.includes('not found') ? 404 : 500).send({
          error: message,
        });
      }
  ...
  server.get('/links', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const query = EnrichmentLinksQuerySchema.parse({
        ...
        enrichmentRecordId: queryParams.enrichmentRecordId as
          | string
          | undefined,
        linkType: queryParams.linkType
          ? Array.isArray(queryParams.linkType)
            ? queryParams.linkType
            : [queryParams.linkType]
          : undefined,
        isMerged:
          queryParams.isMerged === 'true'
            ? true
            : queryParams.isMerged === 'false'
              ? false
              : undefined,
        sortBy: queryParams.sortBy as string | undefined,
        sortOrder: queryParams.sortOrder as string | undefined,
      ...
      return reply.status(500).send({
        error:
          error instanceof Error
            ? error.message
            : 'Failed to list enrichment links',
      });
    }
  });
}
