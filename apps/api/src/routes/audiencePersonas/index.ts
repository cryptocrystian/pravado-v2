...
export default async function audiencePersonasRoutes(server: FastifyInstance) {
  ...
  server.get('/', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const query = PersonasQuerySchema.parse({
        personaType: queryParams.personaType
          ? String(queryParams.personaType).split(',')
          : undefined,
        role: queryParams.role,
        industry: queryParams.industry,
        seniorityLevel: queryParams.seniorityLevel
          ? String(queryParams.seniorityLevel).split(',')
          : undefined,
        minRelevanceScore: queryParams.minRelevanceScore
          ? Number(queryParams.minRelevanceScore)
          : undefined,
        minEngagementScore: queryParams.minEngagementScore
          ? Number(queryParams.minEngagementScore)
          : undefined,
        minAlignmentScore: queryParams.minAlignmentScore
          ? Number(queryParams.minAlignmentScore)
          : undefined,
        minOverallScore: queryParams.minOverallScore
          ? Number(queryParams.minOverallScore)
          : undefined,
        status: queryParams.status
          ? String(queryParams.status).split(',')
          : undefined,
        tags: queryParams.tags
          ? String(queryParams.tags).split(',')
          : undefined,
  ...
  server.get('/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to get persona detail';
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const query = PersonaInsightsQuerySchema.parse({
          insightType: queryParams.insightType
            ? String(queryParams.insightType).split(',')
            : undefined,
          insightCategory: queryParams.insightCategory
            ? String(queryParams.insightCategory).split(',')
            : undefined,
          sourceSystem: queryParams.sourceSystem
            ? String(queryParams.sourceSystem).split(',')
            : undefined,
          minConfidence: queryParams.minConfidence
            ? Number(queryParams.minConfidence)
            : undefined,
          minImpact: queryParams.minImpact
            ? Number(queryParams.minImpact)
            : undefined,
          isActionable:
            queryParams.isActionable === 'true'
              ? true
              : queryParams.isActionable === 'false'
                ? false
                : undefined,
          sortBy: queryParams.sortBy as string | undefined,
          sortOrder: queryParams.sortOrder as string | undefined,
        ...
        const message =
          error instanceof Error
            ? error.message
            : 'Failed to get persona insights';
        return reply.status(500).send({
          error: message,
        });
      }
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const query = PersonaHistoryQuerySchema.parse({
          snapshotType: queryParams.snapshotType
            ? String(queryParams.snapshotType).split(',')
            : undefined,
          minChangeMagnitude: queryParams.minChangeMagnitude
            ? Number(queryParams.minChangeMagnitude)
            : undefined,
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const query = PersonaTrendsQuerySchema.parse({
          daysBack: queryParams.daysBack
            ? Number(queryParams.daysBack)
            : undefined,
          includeTraits: queryParams.includeTraits === 'true',
          includeInsights: queryParams.includeInsights === 'true',
        });

        const result = await service.getPersonaTrends(
          orgId,
          id,
          query.daysBack || 90
        );
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const comparison = await service.comparePersonas(
          orgId,
          personaId1,
          personaId2
        );
        ...
        const message =
          error instanceof Error ? error.message : 'Failed to compare personas';
        return reply.status(message.includes('not found') ? 404 : 500).send({
          error: message,
        });
      }
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Failed to merge personas';
        return reply.status(message.includes('not found') ? 404 : 500).send({
          error: message,
        });
      }
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Failed to add trait';
        return reply.status(message.includes('required') ? 400 : 500).send({
          error: message,
        });
      }
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Failed to add insight';
        return reply.status(message.includes('required') ? 400 : 500).send({
          error: message,
        });
      }
