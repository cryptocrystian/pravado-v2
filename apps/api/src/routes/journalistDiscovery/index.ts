...
export async function journalistDiscoveryRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{
    ...
    async (
      request: FastifyRequest<{ Body: AuthorExtractionInput }>,
      reply: FastifyReply
    ) => {
      try {
        const validationResult = authorExtractionInputSchema.safeParse(
          request.body
        );
        ...
        const result = await discoveryService.extractAuthorsFromArticle(
          orgId,
          input
        );
        ...
        fastify.log.error(
          { error, route: 'POST /journalist-discovery/extract' },
          'Failed to extract authors'
        );
        return reply
          .status(500)
          .send({ error: 'Failed to extract authors', message: error.message });
      }
    }
  );
  ...
  fastify.post<{
    ...
    { onRequest: [requireUser] },
    async (
      request: FastifyRequest<{ Body: DiscoveredJournalistInput }>,
      reply: FastifyReply
    ) => {
      try {
        const validationResult = discoveredJournalistInputSchema.safeParse(
          request.body
        );
        if (!validationResult.success) {
          return reply.status(400).send({
        ...
        const discovery = await discoveryService.createDiscovery(
          orgId,
          input as DiscoveredJournalistInputType
        );

        return reply.status(201).send(discovery);
      } catch (error: any) {
        fastify.log.error(
          { error, route: 'POST /journalist-discovery' },
  ...
  fastify.get<{
    ...
    { onRequest: [requireUser] },
    async (
      request: FastifyRequest<{ Querystring: DiscoveryQuery }>,
      reply: FastifyReply
    ) => {
      try {
        ...
        fastify.log.error(
          { error, route: 'GET /journalist-discovery' },
          'Failed to list discoveries'
        );
        return reply.status(500).send({
          error: 'Failed to list discoveries',
          message: error.message,
        });
      }
    }
  ...
  fastify.get<{
    ...
    async (request, reply: FastifyReply) => {
      try {
        ...
        fastify.log.error(
          { error, route: 'GET /journalist-discovery/stats' },
          'Failed to get discovery stats'
        );
        return reply.status(500).send({
  ...
  fastify.get<{
    ...
    async (
      ...
      try {
        ...
        fastify.log.error(
          { error, route: 'GET /journalist-discovery/:id' },
          'Failed to get discovery'
        );
        return reply
          .status(500)
          .send({ error: 'Failed to get discovery', message: error.message });
      }
    }
  );
  ...
  fastify.put<{
    ...
    async (
      ...
      try {
        ...
        const discoveryService = createJournalistDiscoveryService(supabase);
        const discovery = await discoveryService.updateDiscovery(
          id,
          orgId,
          request.body as Partial<DiscoveredJournalistInputType>
        );
        ...
        fastify.log.error(
          { error, route: 'PUT /journalist-discovery/:id' },
          'Failed to update discovery'
        );
        return reply.status(500).send({
  ...
  fastify.delete<{
    ...
    async (
      ...
      try {
        ...
        fastify.log.error(
          { error, route: 'DELETE /journalist-discovery/:id' },
  ...
  fastify.post<{
    ...
    async (
      ...
      try {
        const validationResult = resolveDiscoveryInputSchema.safeParse(
          request.body
        );
        if (!validationResult.success) {
          return reply.status(400).send({
        ...
        const discoveryService = createJournalistDiscoveryService(supabase);
        const discovery = await discoveryService.resolveDiscovery(
          id,
          orgId,
          user.id,
          input
        ...
        fastify.log.error(
          { error, route: 'POST /journalist-discovery/:id/resolve' },
          'Failed to resolve discovery'
        );
        return reply.status(500).send({
          error: 'Failed to resolve discovery',
          message: error.message,
        });
      }
    }
    ...
    async (
      ...
      try {
        ...
        if (!targetJournalistId) {
          return reply
            .status(400)
            .send({ error: 'targetJournalistId is required' });
        }
        ...
        const discoveryService = createJournalistDiscoveryService(supabase);
        const preview = await discoveryService.generateMergePreview(
          id,
          targetJournalistId,
          orgId
        );
        ...
        fastify.log.error(
          { error, route: 'POST /journalist-discovery/:id/merge-preview' },
  ...
  fastify.post<{
    ...
    async (
      ...
      try {
        ...
        const discoveryService = createJournalistDiscoveryService(supabase);
        const result = await discoveryService.checkDuplication(
          orgId,
          input as DiscoveredJournalistInputType
        );
        ...
        fastify.log.error(
          { error, route: 'POST /journalist-discovery/check-duplication' },
          'Failed to check duplication'
        );
        return reply.status(500).send({
  ...
  fastify.post<{
    ...
    { onRequest: [requireUser] },
    async (
      request: FastifyRequest<{ Body: BatchDiscoveryInput }>,
      reply: FastifyReply
    ) => {
      try {
        const validationResult = batchDiscoveryInputSchema.safeParse(
          request.body
        );
        if (!validationResult.success) {
          return reply.status(400).send({
        ...
        for (let i = 0; i < input.discoveries.length; i++) {
          try {
            // Check duplication
            const dedup = await discoveryService.checkDuplication(
              orgId,
              input.discoveries[i] as DiscoveredJournalistInputType
            );
            ...
            if (
              dedup.isDuplicate &&
              dedup.similarityScore >= (input.autoMergeThreshold || 0.95) &&
              dedup.matchedJournalistId
            ) {
              // Auto-merge
              await discoveryService.createDiscovery(
                orgId,
                input.discoveries[i] as DiscoveredJournalistInputType
              );
              result.merged++;
        ...
        fastify.log.error(
          { error, route: 'POST /journalist-discovery/batch' },
          'Failed to process batch'
        );
        return reply
          .status(500)
          .send({ error: 'Failed to process batch', message: error.message });
      }
    }
  );
  ...
  fastify.post<{
    ...
    { onRequest: [requireUser] },
    async (
      request: FastifyRequest<{ Body: SocialProfileInput }>,
      reply: FastifyReply
    ) => {
      try {
        const validationResult = socialProfileInputSchema.safeParse(
          request.body
        );
        if (!validationResult.success) {
          return reply.status(400).send({
        ...
        const discoveryService = createJournalistDiscoveryService(supabase);
        const discovery = await discoveryService.ingestSocialProfile(
          orgId,
          input
        );
        ...
        fastify.log.error(
          { error, route: 'POST /journalist-discovery/social-profile' },
          'Failed to ingest social profile'
        );
        return reply.status(500).send({
          error: 'Failed to ingest social profile',
          message: error.message,
        });
      }
    }
