 ...
 */
export async function mediaAlertsRoutes(
  ...
  server.get<{
    ...
    async (request, reply) => {
      try {
        ...
        const validation = listMediaAlertRulesQuerySchema.safeParse(
          request.query
        );
        if (!validation.success) {
          return reply.status(400).send({
  ...
  server.get<{
    ...
    async (request, reply) => {
      try {
        ...
        const validation = listMediaAlertEventsQuerySchema.safeParse(
          request.query
        );
        if (!validation.success) {
          return reply.status(400).send({
  ...
  server.post<{
    ...
    async (request, reply) => {
      try {
        ...
        const updatedCount = await alertService.markEventsAsRead(
          orgId,
          validation.data
        );
        ...
        return reply.status(500).send({
          ...
          error: {
            ...
            message:
              error instanceof Error
                ? error.message
                : 'Failed to get signals overview',
          },
        });
      }
    }
