...
export class OpsMetricsService {
   ...
   */
  async getOrgExecutionStats(
    orgId: string,
    period: '24h' | '7d' = '24h'
  ): Promise<OrgExecutionStats> {
    ...
    if (stepFailuresError) {
      throw new Error(
        `Failed to fetch step failures: ${stepFailuresError.message}`
      );
    }
    ...
    const stepFailuresByType = Object.entries(failuresByType).map(
      ([type, count]) => ({
        type,
        count,
      })
  ...
  async getQueueStats(): Promise<QueueStats> {
    ...
    const byType = pendingJobs.reduce(
      (acc, job) => {
  ...
  async getLlmUsageSummary(
    ...
    const byProviderModel = entries.reduce(
      (acc, entry) => {
