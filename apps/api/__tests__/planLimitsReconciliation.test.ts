 ...
 * by the ratified tier order Starter < Pro < Scale. Rate-limit dimensions
...
describe('PR-C — reconciled name-bound values (D030)', () => {
  it('seats match the live pricing page (1 / 5 / 15)', () => {
    ...
    expect(getPlanLimits('scale').seats).toBe(15);
  });
  ...
  it('CRAFT docs/mo match the page (10 / 50 / Unlimited)', () => {
    ...
    expect(getPlanLimits('scale').contentDocumentsPerMonth).toBe(999_999);
  });
  ...
  it('LLM tokens/mo are the canon values billing_plans mirrors (2.5M / 5M / 50M)', () => {
    ...
    expect(getPlanLimits('scale').llmTokensPerMonth).toBe(50_000_000);
  });

  it('is monotonic by ratified tier rank (Starter < Pro < Scale)', () => {
    const s = getPlanLimits('starter');
    const p = getPlanLimits('pro');
    const g = getPlanLimits('scale');
  ...
  it('parked dimensions are UNCHANGED (out of scope — guardrail workstream)', () => {
    ...
    expect(getPlanCeiling('scale')).toBe('autopilot');
