 ...
 *   3. No pitch subject/body → `governed_complete` needs_content (nothing sent, no
 *      fabricated pitch); rawSend never called.
 *   4. Provider attempted the send and reported failure → `failure`.
 *   5. No resolvable recipient → neutral `governed_complete` needs_recipient.
 */
...
const CTX = {
  supabase: {} as SupabaseClient, // never touched: gateways/rawSend/resolve injected
  orgId: 'org-1',
  proposalId: 'prop-1',
  executionId: 'exec-1',
};
...
describe('prSendPitchExecutor', () => {
  ...
  it('missing pitch content → needs_content governed outcome, nothing sent', async () => {
    const { rawSend, calls } = makeRawSend();
    ...
    // Short-circuits before recipient resolution AND before any send.
